...
- 第一版真实 adapter 使用 Hugging Face native `transformers` 路径；不要把 `qwen-asr` 包作为默认加载路径。
...
推荐先用 `Qwen/Qwen3-ASR-0.6B-hf` 和 `test-fixtures/audio/test_short.wav` 验收，降低首次下载和显存压力。`transformers` 返回非空文本后，才能开始接入 `asr_server` adapter。第一版不要在 `/v1/models` 中声明 `vllm`。
